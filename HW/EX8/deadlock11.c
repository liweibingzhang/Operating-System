...
void *threadTamara( void *arg ) {
  while ( running ) {
    ...
    if ( sem_wait( &shovel ) != 0 ) fail( "sem_wait failed" );
    if ( sem_wait( &tricorder ) != 0 ) fail( "sem_wait failed" );
    if ( sem_wait( &bookmark ) != 0 ) fail( "sem_wait failed" );

    // Say my name.
    printf( "Tamara\n" );

    // Release locks on the objects I'm used.
    if ( sem_post( &bookmark ) != 0 ) fail( "sem_post failed" );
    if ( sem_post( &tricorder ) != 0 ) fail( "sem_post failed" );
    if ( sem_post( &shovel ) != 0 ) fail( "sem_post failed" );
  }
  return NULL;
}
...
void *threadAdolfo( void *arg ) {
  while ( running ) {
    // Acquire locks on the objects I'm going to use.
    if ( sem_wait( &bowtie ) != 0 ) fail( "sem_wait failed" );
    if ( sem_wait( &hammer ) != 0 ) fail( "sem_wait failed" );
    if ( sem_wait( &camera ) != 0 ) fail( "sem_wait failed" );
    if ( sem_wait( &usb_cable ) != 0 ) fail( "sem_wait failed" );

    // Say my name.
    printf( "Adolfo\n" );

    // Release locks on the objects I'm used.
    if ( sem_post( &usb_cable ) != 0 ) fail( "sem_post failed" );
    if ( sem_post( &camera ) != 0 ) fail( "sem_post failed" );
    if ( sem_post( &hammer ) != 0 ) fail( "sem_post failed" );
    if ( sem_post( &bowtie ) != 0 ) fail( "sem_post failed" );
  }
  return NULL;
}
...
void *threadTerrie( void *arg ) {
  while ( running ) {
    // Acquire locks on the objects I'm going to use.
    if ( sem_wait( &hammer ) != 0 ) fail( "sem_post failed" );
    if ( sem_wait( &bookmark ) != 0 ) fail( "sem_wait failed" );
    if ( sem_wait( &bowtie ) != 0 ) fail( "sem_wait failed" );
    if ( sem_wait( &rubber_stamp ) != 0 ) fail( "sem_wait failed" );
